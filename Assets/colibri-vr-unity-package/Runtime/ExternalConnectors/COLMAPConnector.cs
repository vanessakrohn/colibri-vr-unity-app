...
namespace COLIBRIVR.ExternalConnectors
{
    ...
    public static class COLMAPConnector
    {
        ...
        public static List<string> COLMAPCameraTypes
        {
            get
            {
                return new List<string>()
                {
                    "SIMPLE_PINHOLE", "PINHOLE", "SIMPLE_RADIAL", "SIMPLE_RADIAL_FISHEYE", "RADIAL", "RADIAL_FISHEYE",
                    "OPENCV", "OPENCV_FISHEYE", "FULL_OPENCV", "FOV", "THIN_PRISM_FISHEYE"
                };
            }
        }

        public static string sparse0DirName
        {
            get { return Path.Combine(COLMAPConnector.sparseDirName, "0"); }
        }

        public static string dense0DirName
        {
            get { return Path.Combine(COLMAPConnector.denseDirName, "0"); }
        }
        ...
        /// <returns></returns>
        public static IEnumerator RunFeatureExtractionCommand(MonoBehaviour caller, string workspace,
            bool displayProgressBar, bool stopOnError, string[] progressBarParams, int COLMAPCameraIndex,
            bool isSingleCamera, float focalLengthFactor)
        {
            ...
            yield return caller.StartCoroutine(GeneralToolkit.RunCommandCoroutine(typeof(COLMAPConnector), command,
                workspace, displayProgressBar, null, null, stopOnError, progressBarParams));
        }
        ...
        /// <param name="progressBarParams"></param> Parameters for the progress bar.
        /// <returns></returns>
        public static IEnumerator RunFeatureMatchingCommand(MonoBehaviour caller, string workspace,
        ...
        /// <returns></returns>
        public static IEnumerator RunMappingCommand(MonoBehaviour caller, string workspace, bool displayProgressBar,
            bool stopOnError, string[] progressBarParams, bool isCameraModelKnown)
        {
        ...
        /// <param name="progressBarParams"></param> Parameters for the progress bar.
        /// <returns></returns>
        public static IEnumerator RunExportModelAsTextCommand(MonoBehaviour caller, string workspace,
            bool displayProgressBar, bool stopOnError, string[] progressBarParams)
        {
        ...
        /// <returns></returns>
        public static IEnumerator RunUndistortionCommand(MonoBehaviour caller, string workspace,
            bool displayProgressBar, bool stopOnError, string[] progressBarParams, int maxImageSize)
        {
        ...
        /// <param name="progressBarParams"></param> Parameters for the progress bar.
        /// <returns></returns>
        public static IEnumerator RunStereoCommand(MonoBehaviour caller, string workspace, bool displayProgressBar,
            bool stopOnError, string[] progressBarParams)
        {
        ...
        /// <param name="progressBarParams"></param> Parameters for the progress bar.
        /// <returns></returns>
        public static IEnumerator RunFusionCommand(MonoBehaviour caller, string workspace, bool displayProgressBar,
            bool stopOnError, string[] progressBarParams)
        {
        ...
        /// <param name="progressBarParams"></param> Parameters for the progress bar.
        /// <returns></returns>
        public static IEnumerator RunDelaunayMeshingCommand(MonoBehaviour caller, string workspace,
            bool displayProgressBar, bool stopOnError, string[] progressBarParams)
        {
        ...
        public static string GetProgressBarParamsOne(string prefix, bool isSparse, int step, int maxStep)
        {
            ...
            return prefix + " - " + typeName + " reconstruction step " + GeneralToolkit.ToString(step) + "/" +
                   GeneralToolkit.ToString(maxStep) + ".";
        }
        ...
        /// <returns></returns>
        public static IEnumerator RunSparseReconstructionCoroutine(COLMAPHelper caller, string workspace,
            int COLMAPCameraIndex, bool isSingleCamera, int maxImageSize)
        {
            ...
            for (int step = 1; step <= maxStep; step++)
            {
                ...
                if (step == 1)
                {
                    ...
                    if (cameraModels != null && cameraModels.Length > 0)
                    {
                        ...
                        float focalLength = Camera.FieldOfViewToFocalLength(cameraParams.fieldOfView.x,
                            cameraParams.pixelResolution.x);
                        focalLengthFactor = focalLength /
                                            Mathf.Max(cameraParams.pixelResolution.x, cameraParams.pixelResolution.y);
                    }

                    yield return caller.StartCoroutine(RunFeatureExtractionCommand(caller, workspace,
                        displayProgressBar, stopOnError, progressBarParams, COLMAPCameraIndex, isSingleCamera,
                        focalLengthFactor));
                }
                ...
                else if (step == 2)
                {
                    ...
                    yield return caller.StartCoroutine(RunFeatureMatchingCommand(caller, workspace, displayProgressBar,
                        stopOnError, progressBarParams));
                }
                // Step three: launch mapping.
                else if (step == 3)
                {
                    progressBarParams[1] = GetProgressBarParamsOne("Mapping", true, step, maxStep);
                    yield return caller.StartCoroutine(RunMappingCommand(caller, workspace, displayProgressBar,
                        stopOnError, progressBarParams, (focalLengthFactor > 0)));
                }
                // Step four: launch exporting original camera setup as text.
                else if (step == 4)
                {
                    progressBarParams[1] = GetProgressBarParamsOne("Exporting camera setup (original) as text", true,
                        step, maxStep);
                    yield return caller.StartCoroutine(RunExportModelAsTextCommand(caller, workspace,
                        displayProgressBar, stopOnError, progressBarParams));
                }
                ...
                else if (step == 5)
                {
                    ...
                    yield return caller.StartCoroutine(RunUndistortionCommand(caller, workspace, displayProgressBar,
                        stopOnError, progressBarParams, maxImageSize));
                }
                // Step six: launch exporting undistorted camera setup as text.
                else if (step == 6)
                {
                    // Launch export process.
                    progressBarParams[1] = GetProgressBarParamsOne("Exporting camera setup (undistorted) as text", true,
                        step, maxStep);
                    yield return caller.StartCoroutine(RunExportModelAsTextCommand(caller, GetDense0Dir(workspace),
                        displayProgressBar, stopOnError, progressBarParams));
                    ...
                    Debug.Log(GeneralToolkit.FormatScriptMessage(typeof(COLMAPConnector),
                        "Changed data directory to: " + workspace + "."));
                    yield return null;
                }
            ...
            {
                Debug.Log(GeneralToolkit.FormatScriptMessage(typeof(COLMAPConnector),
                    "Sparse reconstruction was a success."));
        ...
        /// <returns></returns>
        public static IEnumerator UpdateDenseReconstructionProgressBasedOnDepthMapCount(string workspace,
            int imageCount, string[] progressBarParams)
        {
            ...
            while (currentDepthMapCount < expectedDepthMapCount)
            {
                ...
                if (newDepthMapCount > currentDepthMapCount)
                    for (int iter = currentDepthMapCount; iter < newDepthMapCount; iter++)
                        GeneralToolkit.UpdateCancelableProgressBarWithParams(typeof(COLMAPConnector),
                            progressBarParams);
            ...
            int imageCount)
        {
            ...
            for (int step = 1; step <= maxStep; step++)
            {
                ...
                if (step == 1)
                {
                    ...
                    caller.StartCoroutine(
                        UpdateDenseReconstructionProgressBasedOnDepthMapCount(workspace, imageCount,
                            progressBarParams));
                    yield return caller.StartCoroutine(RunStereoCommand(caller, workspace, displayProgressBar,
                        stopOnError, progressBarParams));
                }
                ...
                else if (step == 2)
                {
                    ...
                    yield return caller.StartCoroutine(RunFusionCommand(caller, workspace, displayProgressBar,
                        stopOnError, progressBarParams));
                }
                ...
                else if (step == 3)
                {
                    ...
                    yield return caller.StartCoroutine(RunDelaunayMeshingCommand(caller, workspace, displayProgressBar,
                        stopOnError, progressBarParams));
                }
            ...
            if (!GeneralToolkit.progressBarCanceled)
                Debug.Log(GeneralToolkit.FormatScriptMessage(typeof(COLMAPConnector),
                    "Dense reconstruction was a success."));
            // Indicate to the user that the process has ended.
            GeneralToolkit.ResetCancelableProgressBar(false, false);
        }
            ...
            else
            {
                ...
                if (cameraType == "SIMPLE_PINHOLE" || cameraType == "PINHOLE" || cameraType == "OMNIDIRECTIONAL")
                {
                    if (cameraType == "SIMPLE_PINHOLE" && numberOfParameters == 5)
                        ...
                        warning = "Provided number of parameters (" + numberOfParameters +
                                  ") is not the expected number for camera type " + cameraType;
                }
        ...
        private static CameraModel BasicParse(string[] split, bool isOmnidirectional)
        {
            ...
            cameraModel.pixelResolution =
                new Vector2Int(GeneralToolkit.ParseInt(split[2]), GeneralToolkit.ParseInt(split[3]));
        ...
        public static void SaveCamerasInformation(CameraModel[] cameraModels, string workspace)
        {
            ...
            for (int iter = 0; iter < cameraModels.Length; iter++)
            {
                ...
                string PARAMS_FOCALLENGTH =
                    GeneralToolkit.ToString(Camera.FieldOfViewToFocalLength(cameraModel.fieldOfView.x,
                        cameraModel.pixelResolution.x));
                ...
                string line = CAMERA_ID + " " + MODEL + " " + WIDTH + " " + HEIGHT + " " + PARAMS_FOCALLENGTH + " " +
                              PARAMS_CENTERWIDTH + " " + PARAMS_CENTERHEIGHT;
        ...
        public static void ReadCamerasInformation(CameraSetup cameraSetup, string workspace)
        {
            ...
            ReadCamerasInformationGivenLines(cameraSetup, lines);
        }

        public static void ReadCamerasInformationGivenLines(CameraSetup cameraSetup, string[] lines)
        {
            ...
            foreach (string line in lines)
            {
                if (!line.StartsWith("#") && line.Length > 0)
                {
                    ...
                    if (cameraModel == null)
                    {
                        ...
                        warning +=
                            " If an unsupported COLMAP camera model was recognized, try navigating to {current_directory}/dense/0.";
        ...
        public static void SaveImagesInformation(CameraModel[] cameraModels, string workspace)
        {
            ...
            for (int i = 0; i < cameraModels.Length; i++)
            {
                ...
                string line = IMAGE_ID + " " + QW + " " + QX + " " + QY + " " + QZ + " " + TX + " " + TY + " " + TZ +
                              " " + CAMERA_ID + " " + NAME;
        ...
        public static void ReadImagesInformation(CameraSetup cameraSetup, string workspace,
            out bool pointCorrespondencesExist)
        {
            // Read COLMAP's images file.
            using StreamReader reader = File.OpenText(GetImagesFile(workspace));
            ReadImagesInformationGivenReader(cameraSetup, reader, out pointCorrespondencesExist);
        }

        public static void ReadImagesInformationGivenReader(CameraSetup cameraSetup, StreamReader reader,
            out bool pointCorrespondencesExist)
        {
                ...
                if (!line.StartsWith("#"))
                {
                    ...
                    else
                    {
                        ...
                        if (split.Length > 9)
                        {
                            ...
                            Quaternion rotation = new Quaternion(GeneralToolkit.ParseFloat(split[2]),
                                GeneralToolkit.ParseFloat(split[3]), GeneralToolkit.ParseFloat(split[4]),
                                GeneralToolkit.ParseFloat(split[1]));
                            Vector3 position = new Vector3(GeneralToolkit.ParseFloat(split[5]),
                                GeneralToolkit.ParseFloat(split[6]), GeneralToolkit.ParseFloat(split[7]));
            ...
            for (int sourceCamIndex = 0; sourceCamIndex < positionList.Count; sourceCamIndex++)
            {
                ...
                cameraModel.SetCameraReferenceIndexAndImageName(cameraIDList[sourceCamIndex],
                    fileNameList[sourceCamIndex]);
