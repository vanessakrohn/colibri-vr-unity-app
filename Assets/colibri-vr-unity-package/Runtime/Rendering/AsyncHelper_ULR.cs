...
namespace COLIBRIVR.Rendering
{
    ...
    /// </summary>
    public class AsyncHelper_ULR : AsyncMethod
    {
...
#region FIELDS
        ...
        [SerializeField] public float _depthCorrectionFactor;
...
#region INHERITANCE_METHODS
        ...
        void OnRenderObject()
        {
            ...
            if(!_helperULRCameras.Contains(currentCamera))
            {
                ...
                {
                    var collider = processingCaller.gameObject.GetComponent<MeshCollider>();
                    collider.sharedMesh = PMGlobalMeshEF.globalMesh;
                    AsyncHelper_ULRCamera helperULRCamera = GeneralToolkit.GetOrAddComponent<AsyncHelper_ULRCamera>(currentCamera.gameObject);
...
#region METHODS
        ...
        public AsyncProcessingMethod[] GetULRSceneRepresentationMethods()
        {
            // Note that the global texture map is optional.
            return new AsyncProcessingMethod[] { PMColorTextureArray, PMGlobalMeshEF, PMDepthTextureArray };
        }
