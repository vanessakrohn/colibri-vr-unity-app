...
namespace COLIBRIVR.Rendering
{
    ...
    /// </summary>
    [RequireComponent(typeof(Processing.AsyncProcessing))]
    public class AsyncRendering : MonoBehaviour
    {
        public string url;
        private float _depthCorrectionFactor;
        ...
        public static int renderingObjectCount
        {
            get { return _renderingObjectList.Count; }
        }
        ...
        private static List<AsyncRendering> _renderingObjectList;
        ...
        public static void ChangeIndexInQueue(AsyncRendering renderingObj, int newIndex)
        {
        ...
        private static void UpdateRenderingObjectsList()
        {
            if (_renderingObjectList == null)
                _renderingObjectList = new List<AsyncRendering>();
            AsyncRendering[] renderingObjArray = FindObjectsOfType<AsyncRendering>();
        ...
        private static void LaunchRenderingObjects()
        {
            ...
            {
                AsyncRendering renderingObj = _renderingObjectList[iter];
        ...
        public Processing.AsyncProcessing processing
        {
            get { return _processing; }
        }

        public AsyncRenderingMethod[] renderingMethods
        {
            get { return _renderingMethods; }
        }

        public EvaluationMethod[] evaluationMethods
        {
            get { return _evaluationMethods; }
        }
        ...
        public AsyncRenderingMethod selectedBlendingMethod;
        ...
        [SerializeField] private Processing.AsyncProcessing _processing;
        [SerializeField] private int _renderingMethodIndex;
        [SerializeField] private int _evaluationMethodIndex;
        [SerializeField] private AsyncRenderingMethod[] _renderingMethods;
        ...
        public IEnumerator LaunchRenderingWithURL(string newUrl)
        {
            url = newUrl;
            readyToBeLaunched = true;
            yield return LaunchRenderingCoroutine();
        }
        ...
        void Reset()
        {
            ...
            _processing = GeneralToolkit.GetOrAddComponent<Processing.AsyncProcessing>(gameObject);
            ...
            _renderingMethods = AsyncRenderingMethod.CreateOrResetRenderingMethods(transform);
            AsyncMethod[] methods = GetComponentsInChildren<AsyncMethod>();
        ...
        public void LoadRenderedViewAsPreview()
        {
            ...
            if (processing.cameraSetup.cameraModels == null || !Application.isPlaying ||
                !gameObject.activeInHierarchy || !_launched)
                return;
            ...
            tempCameraModel.ParametersFromCameraModel(
                processing.cameraSetup.cameraModels[processing.cameraSetup.previewIndex]);
            ...
            if (selectedBlendingMethod != null && selectedEvaluationMethod != null)
            {
                ...
                PreviewCameraManager previewCameraManager =
                    new GameObject("Preview Camera Manager").AddComponent<PreviewCameraManager>();
                Transform previewCameraTransform = new GameObject("Preview Camera").transform;
                GeneralToolkit.CreateRenderTexture(ref previewCameraManager.targetTexture, Vector2Int.one, 0,
                    RenderTextureFormat.ARGB32, false, FilterMode.Point, TextureWrapMode.Clamp);
                previewCameraManager.CreatePreviewCamera(previewCameraManager.gameObject, previewCameraTransform,
                    tempCameraModel);
                ...
                GeneralToolkit.CreateRenderTexture(ref _previewRenderTexture, tempCameraModel.pixelResolution, 0,
                    RenderTextureFormat.ARGB32, false, FilterMode.Point, TextureWrapMode.Clamp);
            ...
            if (selectedEvaluationMethod != null && selectedEvaluationMethod.evaluationMaterial != null)
            {
                ...
                GeneralToolkit.CreateRenderTexture(ref _previewEvalTexture, tempCameraModel.pixelResolution, 0,
                    RenderTextureFormat.ARGB32, true, FilterMode.Point, TextureWrapMode.Clamp);
                selectedEvaluationMethod.evaluationMaterial.SetTexture(EvaluationMethod.shaderNameTextureOne,
                    processing.previewSourceTexture);
                selectedEvaluationMethod.evaluationMaterial.SetTexture(EvaluationMethod.shaderNameTextureTwo,
                    _previewRenderTexture);
        ...
        private IEnumerator ReadAcquisitionInformationRuntimeCoroutine()
        {
            // Reset the setup's camera models
            processing.cameraSetup.ResetCameraModels();

            // Read images information
            var www = UnityWebRequest.Get(url + "images.txt");
            yield return www.SendWebRequest();
            byte[] results = www.downloadHandler.data;
            using var stream = new MemoryStream(results);
            using var reader = new StreamReader(stream);
            COLMAPConnector.ReadImagesInformationGivenReader(processing.cameraSetup, reader,
                out processing.dataHandler.imagePointCorrespondencesExist);

            processing.dataHandler.SetImageURLs(processing.cameraSetup);

            // Read cameras information
            www = UnityWebRequest.Get(url + "cameras.txt");
            yield return www.SendWebRequest();
            COLMAPConnector.ReadCamerasInformationGivenLines(processing.cameraSetup,
                www.downloadHandler.text.Split('\n'));

            if (processing.cameraSetup.cameraModels != null && processing.cameraSetup.cameraModels.Length > 0)
            {
                www = UnityWebRequest.Get(url + "additional_information.txt");
                yield return www.SendWebRequest();
                processing.dataHandler.ReadCOLIBRIVRAdditionalInformationGivenLines(processing.cameraSetup,
                    www.downloadHandler.text.Split('\n'));
            }

            // Read params
            www = UnityWebRequest.Get(url + "params.json");
            yield return www.SendWebRequest();
            var json = www.downloadHandler.text;
            
            var parsedParams = JsonUtility.FromJson<AsyncRenderingParams>(json);
            _depthCorrectionFactor = parsedParams.depthCorrectionFactor;
            transform.localScale = new Vector3(parsedParams.scaleX,parsedParams.scaleY,parsedParams.scaleZ);
        }
        ...
        private IEnumerator LaunchRenderingCoroutine()
        {
            ...
            yield return StartCoroutine(ReadAcquisitionInformationRuntimeCoroutine());
            ...
            // Always use ULR blending method
            var urlBlendingMethod = (AsyncULRGlobalMesh)renderingMethods[1];
            urlBlendingMethod._helperULR._depthCorrectionFactor = _depthCorrectionFactor;

            selectedBlendingMethod = urlBlendingMethod;
            selectedEvaluationMethod = evaluationMethods[0];
            ...
            Debug.Log(GeneralToolkit.FormatScriptMessage(this.GetType(),
                "Finished initialization for object " + gameObject.name + "."));
        ...
        #endregion //METHODS
        [System.Serializable]
        private class AsyncRenderingParams
        {
            public float depthCorrectionFactor;
