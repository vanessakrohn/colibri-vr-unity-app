...
namespace COLIBRIVR
{
    ...
    public class DataHandler : MonoBehaviour
    {
        ...
        {
            get { return Path.Combine(GeneralToolkit.GetProjectDirectoryPath(), "Data"); }
        }
        ...
        {
            get { return COLMAPConnector.GetImagesDir(dataDirectory); }
        }

        public string depthDirectory
        {
            get { return COLMAPConnector.GetDepthMapsDir(dataDirectory); }
        }

        public string processedDataDirectory
        {
            get { return Path.Combine(dataDirectory, "processed_data"); }
        }

        public string processingInfoFileName
        {
            get { return "processing_information.txt"; }
        }

        public string processingInfoFilePath
        {
            get { return Path.Combine(processedDataDirectory, processingInfoFileName); }
        }

        public string bundleDirectory
        {
            get { return Path.Combine(dataDirectory, "bundled_data"); }
        }

        public string additionalInfoFile
        {
            get { return Path.Combine(dataDirectory, "additional_information.txt"); }
        }
        ...
        public void SaveAdditionalSetupInformation(CameraSetup cameraSetup)
        {
            ...
            string line = GeneralToolkit.ToString(initialViewingPos.x) + " " +
                          GeneralToolkit.ToString(initialViewingPos.y) + " " +
                          GeneralToolkit.ToString(initialViewingPos.z);
            ...
            foreach (CameraModel camera in cameraSetup.cameraModels)
            {
                ...
                stringBuilder.AppendLine(GeneralToolkit.ToString(camera.cameraReferenceIndex) + " " +
                                         GeneralToolkit.ToString(distanceRange.x) + " " +
                                         GeneralToolkit.ToString(distanceRange.y));
            }
        ...
        public bool CreateAssetBundleFromCreatedAssets()
        {
            ...
            GeneralToolkit.Replace(PathType.Directory, processedDataDirectory,
                GeneralToolkit.tempDirectoryAbsolutePath);
        ...
        public void SectionGenerateColliders()
        {
            ...
            SerializedProperty propertyGenerateColliders =
                serializedObject.FindProperty(_propertyNameGenerateColliders);
            propertyGenerateColliders.boolValue =
                EditorGUILayout.Toggle(new GUIContent(label, tooltip), propertyGenerateColliders.boolValue);
            serializedObject.ApplyModifiedProperties();
        }
        ...
        public void SetImageURLs(CameraSetup cameraSetup)
        {
            ...
            {
                cameraSetup.cameraModels[iter].imageURL =
                    Path.Combine(colorDirectory, cameraSetup.cameraModels[iter].imageName);
            }
        }
        ...
        public void ReadCOLIBRIVRAdditionalInformation(CameraSetup cameraSetup)
        {
            ...
            ReadCOLIBRIVRAdditionalInformationGivenLines(cameraSetup, lines);
        }

        public void ReadCOLIBRIVRAdditionalInformationGivenLines(CameraSetup cameraSetup, string[] lines)
        {
            for (int i = 0; i < lines.Length; ++i)
            {
                if (lines[i].Contains("INITIAL_VIEWING_POSITION"))
                {
                    ...
                    if (split.Length == 3)
                    {
                        Vector3 newInitialViewingPosition = new Vector3(GeneralToolkit.ParseFloat(split[0]),
                            GeneralToolkit.ParseFloat(split[1]), GeneralToolkit.ParseFloat(split[2]));
                ...
                if (lines[i].Contains("DISTANCE_RANGE"))
                {
                    while (i + 1 < lines.Length && !lines[i + 1].StartsWith("#"))
                    {
                        ...
                        if (split.Length == 3)
                        {
                            ...
                            Vector2 newDistanceRange = new Vector2(GeneralToolkit.ParseFloat(split[1]),
                                GeneralToolkit.ParseFloat(split[2]));
        ...
        public void CheckStatusOfSourceData()
        {
            ...
            if (_processingCaller != null)
            {
                string colorInfo = sourceColorCount + " color image" + ((sourceColorCount == 1) ? string.Empty : "s") +
                                   ", ";
                string depthInfo = sourcePerViewCount + " depth map" +
                                   ((sourcePerViewCount == 1) ? string.Empty : "s") + ", ";
        ...
        public void CheckStatusOfDataProcessingAndBundling(out bool isReadyForBundling, out bool isBundled,
            out string summaryInfo)
        {
            isBundled = false;
        ...
        public bool IsAssetAlreadyProcessed(string bundledAssetName)
        {
            ...
            {
                string debug = "Skipping asset \"" + bundledAssetName +
                               "\" because it is already processed. To overwrite it, delete it first.";
        ...
        public void UpdateProcessedAssets()
        {
            ...
            for (int i = 0; i < processedAssetFiles.Length; i++)
                stringBuilder.AppendLine(processedAssetFiles[i].Name
                    .Replace(processedAssetFiles[i].Extension, string.Empty));
        ...
        /// <returns></returns>
        public IEnumerator LoadAssetsFromBundleCoroutine<T>(System.Action<T[]> OutputAssets,
            params string[] bundledAssetsNames) where T : UnityEngine.Object
        {
            ...
            yield return StartCoroutine(
                GeneralToolkit.LoadAssetsFromBundleInMemory<T>((results => OutputAssets(results)), bundledAssetsNames));
        }
        
        public IEnumerator LoadAssetsFromBundleFromUrlCoroutine<T>(string url, System.Action<T[]> OutputAssets,
            params string[] bundledAssetsNames) where T : UnityEngine.Object
        {
            yield return StartCoroutine(GeneralToolkit.LoadAssetBundleIntoMemoryFromUrl(url));
            yield return StartCoroutine(
                GeneralToolkit.LoadAssetsFromBundleInMemory<T>((results => OutputAssets(results)), bundledAssetsNames));
        }
