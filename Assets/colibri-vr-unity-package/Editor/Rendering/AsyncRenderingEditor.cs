...
namespace COLIBRIVR.Rendering
{
    ...
    /// </summary>
    [CustomEditor(typeof(AsyncRendering))]
    public class AsyncRenderingEditor : Editor
    {

#region FIELDS

        private AsyncRendering _targetObject;
...
#region INHERITANCE_METHODS
        ...
        void OnEnable()
        {
            _targetObject = (AsyncRendering)serializedObject.targetObject;
            // Get the target properties.
            _propertyLaunchOrderIndex = serializedObject.FindProperty(AsyncRendering.propertyNameLaunchOrderIndex);
            _propertyLaunchOnAwake = serializedObject.FindProperty(AsyncRendering.propertyNameLaunchOnAwake);
            _propertyRenderingMethodIndex = serializedObject.FindProperty(AsyncRendering.propertyNameRenderingMethodIndex);
            _propertyEvaluationMethodIndex = serializedObject.FindProperty(AsyncRendering.propertyNameEvaluationMethodIndex);
        }
...
#region METHODS
        ...
        private void SectionLaunchOptions()
        {
            ...
            if(_propertyLaunchOnAwake.boolValue)
            {
                ...
                int newLaunchOrderIndex = EditorGUILayout.IntSlider(new GUIContent(label, tooltip), _propertyLaunchOrderIndex.intValue, 0, AsyncRendering.renderingObjectCount - 1);
                if(_propertyLaunchOrderIndex.intValue != newLaunchOrderIndex)
                {
                    AsyncRendering.ChangeIndexInQueue(_targetObject, newLaunchOrderIndex);
        ...
        {
            AsyncRenderingMethod[] compatibleBlendingMethods = AsyncRenderingMethod.GetCompatibleRenderingMethods(_targetObject);
            string label = "Type:";
            string tooltip = "Choice of a blending method.";
            _targetObject.selectedBlendingMethod = DisplayMethodGUI<AsyncRenderingMethod>(label, tooltip, _targetObject.renderingMethods, compatibleBlendingMethods, _propertyRenderingMethodIndex);
            ...
            AsyncProcessingMethod[] sceneRepresentationMethods = _targetObject.selectedBlendingMethod.sceneRepresentationMethods;
            foreach(AsyncProcessingMethod method in sceneRepresentationMethods)
            {
                if(method == null)
                    continue;
                using (var horizontalScope = new EditorGUILayout.HorizontalScope())
